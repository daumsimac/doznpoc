...
<v-card style="width:300px; margin-left:5%;" outlined>
    ...
    <v-card-title v-if="value._links">
        DepositReply # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
    </v-card-title >
    <v-card-title v-else>
        DepositReply
    </v-card-title >
    ...
    <v-card-text style = "margin-left:-15px; margin-top:10px;">
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="BnkC" v-model="value.bnkC"/>
          </div>
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="OrgC" v-model="value.orgC"/>
          </div>
          <div class="grey--text ml-4" v-else>
            OrgC :  {{value.orgC }}
          </div>
          ...
          <div class="grey--text ml-4" v-if="editMode" style = "margin-top:-20px;">
            <v-text-field label="TgrmDsc" v-model="value.tgrmDsc"/>
          </div>
          <div class="grey--text ml-4" v-else>
            TgrmDsc :  {{value.tgrmDsc }}
          </div>
    ...
    methods: {
      ...
      async save(){
        try{
          var temp = null;
          if(this.isNew){
            temp = await axios.post(axios.fixUrl('/depositReplies'), this.value)
          }else{
